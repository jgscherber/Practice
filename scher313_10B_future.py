...
class Inventory(object):
    ...
    def addPet(self, newPet):
        self.animals.append(newPet)
    def Display(self):
        for ani in self.animals:
            ani.Display()
            print("{0:-<35}".format(""))
def main():
    # maybe loop over a list of variables to allow checking for null string
    # easier
    while True:
        pType = input("Enter the type of pet: ")
        if pType = "":
            break
        pName = input("Enter the pet's name: ")
        pSex = input("Enter the pet's sex (M/F): ")
        pAge = input("Enter the pet's age (years): ")
        pAdoptFee = input("Enter cost of adoption ($): ")
        pFixed = input("Is the pet neutered or spayed (T/F): ")
        if pFixed in ['t','T','True','true', 'y','Y','yes','Yes']:
            pFixed = True
        else: pFixed = False
...
test4 = Inventory([test1,test2,test3])
