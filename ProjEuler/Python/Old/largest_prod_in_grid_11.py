...
# vertically, horizontal, diagonally
# put into a matrix
# horizontal will have same row, columns n to (n+4)
# veritcle will have same column, rows m to (m+4)
# diagonals wills have m = n, (m+x)=n, m=(n+y)
#   upper-triangle: m = (n+y)
#   lower-triangle: (m+x) = n
#   diagonal: m = n
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"""
    ...
    for m in range(20):
        for n in range(20):
            grid[m][n] = int(grid[m][n])
    return grid

def largest_horizontal(matrix):
    largest = 0
    for m in matrix:
        for i in range(17):
...
def largest_diagonal(matrix):
    ...
    # diagonal
    for m in range(17):
        product = matrix[m][m]*matrix[m+1][m+1]*matrix[m+2][m+2]*matrix[m+3][m+3]
        if product > largest:
            largest = product
    # upper-triangle
    for x in range(1,17):
        for m in range(17):
            if (m+x+3) < 20:
                product = matrix[m][m+x]*matrix[m+1][m+x+1]\
                          *matrix[m+2][m+x+2]*matrix[m+3][m+x+3]
            if product > largest:
                largest = product
    # lower-triangle                
    for y in range(1,17):
        for m in range(17):
            if (m+y+3) < 20:
                product = matrix[m+y][m]*matrix[m+y+1][m+1]\
                          *matrix[m+y+2][m+2]*matrix[m+y+3][m+3]
            if product > largest:
                largest = product
    return largest
# need reverse diagonal (bottom-left to upper-right)
